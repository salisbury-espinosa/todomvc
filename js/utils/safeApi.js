...
class RequestToApi {
  constructor(requestConfig, callback) {
  ...
  _preprocessRequest() {
    if (!(this.requestConfig.headers && this.requestConfig.headers.authorization)) {
      return this.requestConfig
    }
    this.requestConfig.headers['Content-Type'] = 'text/plain'
    try {
      let symmetricKeys = getSymmetricKeys()
      // TODO query params decryption
      let query = this.requestConfig.url.split('?')
      if (query[1]) {
        ...
        this.requestConfig.url = query[0] + '?' + encodeURIComponent(encryptedQuery)
      }
      if (this.requestConfig.data) {
        ...
        /*jscs:disable requireCamelCaseOrUpperCaseIdentifiers*/
        this.requestConfig.data = new Buffer(sodium.crypto_secretbox_easy(data,
          symmetricKeys.nonce, symmetricKeys.key)).toString('base64')
        /*jscs:enable requireCamelCaseOrUpperCaseIdentifiers*/
      }
      return this.requestConfig
  ...
  send() {
    this._preprocessRequest(this.requestConfig)
...
function sendAuthorisationRequest(callback) {
  ...
  (new RequestToApi(requestConfig, onResponse)).send()
}
...
function isTokenValid(callback) {
  ...
  (new RequestToApi(requestConfig, callback)).send()
}
